CLASS zcl_data_generation_8191 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_data_generation_8191 IMPLEMENTATION.
  METHOD if_oo_adt_classrun~main.
    DELETE FROM ztravel_8191_a.
    INSERT ztravel_8191_a FROM (
        ...
        DELETE FROM zbooking_8191_a.
        INSERT zbooking_8191_a FROM (
        SELECT
            FROM /dmo/booking
            JOIN ztravel_8191_a ON /dmo/booking~travel_id = ztravel_8191_a~travel_id
            JOIN /dmo/travel ON /dmo/travel~travel_id = /dmo/booking~travel_id
            FIELDS "Client
                uuid( ) AS booking_uuid,
                ztravel_8191_a~travel_uuid AS parent_uuid,
                ...
                ztravel_8191_a~last_changed_at as local_last_changed_at ).

         DELETE FROM zbksppl_8191_a.
         out->write( 'Adding Booking Supplements data' ).
         INSERT zbksppl_8191_a FROM (
           SELECT FROM /dmo/book_suppl AS supp
                   JOIN ztravel_8191_a AS trvl ON trvl~travel_id = supp~travel_id
                   JOIN zbooking_8191_a AS book ON book~parent_uuid = trvl~travel_uuid
